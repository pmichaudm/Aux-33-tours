...
class GetUsedVinyl:
    ...
    def set_random_vinyl(self):
        ...
        with open(f'csv/records/New-Arrivals-Used/{new_arrivals[0]}', 'r') as f:
            ...
            self.df = pd.DataFrame(reader, columns=['name', 'price', 'link', 'genre', 'record_label', 'record_country', 'record_cat', 'record_grade', 'sleeve_grade', 'record_info']).sample()
            ...
            self.record_label = self.df['record_label'].to_string(index=False, header=False)
            self.record_country = self.df['record_country'].to_string(index=False, header=False)
            self.record_cat = self.df['record_cat'].to_string(index=False, header=False)
            self.record_grade = self.df['record_grade'].to_string(index=False, header=False)
            self.sleeve_grade = self.df['sleeve_grade'].to_string(index=False, header=False)
            self.record_info = self.df['record_info'].to_string(index=False, header=False)
            f.close()
